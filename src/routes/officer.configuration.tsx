  ...
  const configs = [
    {
      ...
      icon: Shield,
      items: [
        { label: "Railway Rule Version", value: config.ruleVersion, hint: "Statutory rules engine baseline" },
        { label: "Last Rules Ref Update", value: config.lastFormulaUpdate, hint: "Revision synchronization date" },
      ],
    },
    {
      ...
      icon: Cpu,
      items: [
        { label: "Formula Version", value: config.formulaVersion, hint: "Mathematical calculation model code version" },
        { label: "Current Pay Matrix", value: config.currentPayMatrix, hint: "Admissible salary matrix tiers" },
      ],
    },
    {
      ...
      icon: Database,
      items: [
        { label: "Database Status", value: config.databaseStatus, badge: "Active", hint: "Storage engine status" },
        { label: "AI Assistance Engine", value: config.aiModel, hint: "Large Language Model backing query indexer" },
      ],
    },
    {
      ...
      icon: Eye,
      items: [
        { label: "Accessibility Standard", value: config.accessibilityStatus, badge: "Compliant", hint: "UI inclusion parameters" },
        { label: "Application Version", value: config.applicationVersion, hint: "Build version for demo verification" },
      ],
    },
  ];
              ...
              <div className="space-y-4">
                {grp.items.map((item) => (
                  ...
                  >
                    ...
                    {item.badge && (
                      <Badge variant="outline" className="h-6 text-[10px] font-bold bg-emerald-500/10 text-emerald-500 border-emerald-500/20 shrink-0 self-start sm:self-center">
                        {item.badge}
                      </Badge>
                    )}
                  </div>
      ...
      <SectionCard>
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 rounded-lg bg-red-500/5 border border-red-500/10">
          <div className="space-y-1 text-center sm:text-left">
            ...
            <p className="text-xs text-muted-foreground">Re-seeding will clear document uploads and restore default rules parameters.</p>
          </div>
          <button
            onClick={() => {
