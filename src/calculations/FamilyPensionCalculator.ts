...
export class FamilyPensionCalculator extends BaseBenefitCalculator {
  ...
  calculate(context: CalculationContext): BenefitCalculation {
    const result = context.ruleResult.benefitResults.find((benefit) => benefit.benefitName === "Family Pension");
    const eligible = result?.eligibility === "Eligible";
    const basicPay = context.assessment.salaryDetails.currentBasicPay;
    const ordinaryFamilyPension = basicPay * 0.3;
    const enhancedFamilyPension = basicPay * 0.5;
    ...
    return {
      ...
      reason: eligible ? "Death case selected. Family Pension calculated from Last Drawn Basic Pay." : "Family Pension applies only to death cases.",
      formula: this.formulaReference(),
      warnings: eligible ? ["Family and nominee details must be verified before final authorization."] : [],
      details: {
