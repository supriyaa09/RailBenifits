...
function calcBasicPension(context: CalculationContext, decision: RetirementRuleDecision): BenefitCalculation {
...
function calcFamilyPension(context: CalculationContext, decision: RetirementRuleDecision): BenefitCalculation {
  const { assessment } = context;
...
function calcRetirementGratuity(context: CalculationContext, decision: RetirementRuleDecision): BenefitCalculation {
  const { assessment } = context;
  ...
  if (!admissible) {
    return notEligible("retirementGratuity", "Retirement Gratuity", decision.reason, "RETIREMENT_GRATUITY");
  }
...
function calcLeaveEncashment(context: CalculationContext, decision: RetirementRuleDecision): BenefitCalculation {
  const { assessment } = context;
  const admissible = decision.benefits["leaveEncashment" as SettlementBenefitKey];
  if (!admissible) {
    return notEligible("leaveEncashment", "Leave Encashment", decision.reason, "OPS_LEAVE_ENCASHMENT");
  }
  ...
  const effectiveLhapDays = Math.min(convertedLhapDays, remainingDays, LEAVE_RULES.maxEncashableLhapDays);
  ...
  if (lapDays + lhapDays / LEAVE_RULES.lhapConversionDivisor > LEAVE_RULES.maxTotalEncashableDays) {
    warnings.push(`Combined LAP and converted LHAP days exceed the maximum limit of ${LEAVE_RULES.maxTotalEncashableDays} encashable days.`);
  }
  if (convertedLhapDays > LEAVE_RULES.maxEncashableLhapDays && effectiveLhapDays === LEAVE_RULES.maxEncashableLhapDays) {
    warnings.push(`Encashable LHAP days are capped at the maximum limit of ${LEAVE_RULES.maxEncashableLhapDays} days.`);
  }
  ...
  return benefit(
    ...
    amount,
    true,
    `Leave Encashment = (Basic + DA) × ${totalEncashableDays} days / ${LEAVE_RULES.monthDivisor}`,
...
function calcRELHS(context: CalculationContext, decision: RetirementRuleDecision): BenefitCalculation {
  const { assessment } = context;
...
function calcFMA(context: CalculationContext, decision: RetirementRuleDecision): BenefitCalculation {
  const { assessment } = context;
...
function calcCTG(context: CalculationContext, decision: RetirementRuleDecision): BenefitCalculation {
  const { assessment } = context;
...
function calcCommutation(context: CalculationContext, decision: RetirementRuleDecision, basicPensionAmount: number): BenefitCalculation {
  const { assessment } = context;
  const admissible = decision.benefits["commutation" as SettlementBenefitKey];
  if (!admissible) {
    return notEligible("commutation", "Commutation", decision.reason, "COMMUTATION_FACTOR_BY_AGE_NEXT_BIRTHDAY");
  }

  const opted = assessment.commutationDetails.commutationOpted;
  if (!opted) {
    return notEligible("commutation", "Commutation", "Employee did not opt for commutation.", "COMMUTATION_FACTOR_BY_AGE_NEXT_BIRTHDAY");
  }
...
export function calculateWithFormulaEngine(
  ...
  const totalEstimatedSettlement = totalOneTimeBenefits + monthlyPension * 12 * 10 + monthlyFma * 12 * 10;
